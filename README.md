...
- *Single injection magnitude:* the synthetic evaluation tested one spike value (2.0) and one drop value (-0.5) rather than sweeping across a range of magnitudes. The drop value of -0.5 was chosen to simulate realistic low-value anomalies such as refunds, reversed charges, or near-zero missed payments — a subtle drop rather than a catastrophically wrong value. A more rigorous evaluation would test multiple injection magnitudes (e.g. spikes at 1.2, 1.5, 2.0, 3.0 and drops at -0.1, -0.3, -0.5, -1.5) to produce a sensitivity curve showing at what point the model reliably begins catching each anomaly type. The current 30.3% drop recall is an honest reflection of the model struggling with mild low-value anomalies, not a failure to detect extreme ones.
